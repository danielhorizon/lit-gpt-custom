...
def fit(
    ...
    max_iters = train.max_tokens // train.global_batch_size
    train_iter = CycleIterator(train_dataloader)
    val_iter = CycleIterator(val_dataloader)
    throughput = ThroughputMonitor(fabric, window_size=50)
    running_loss = RunningMean(window=train.log_interval, sync_on_compute=False).to(fabric.device)
    best_val_loss = float("inf")
    train_step = 0

    # resume training loop
    for step_count in range(state["iter_num"], max_iters):
        if train_step >= max_iters:
            break

        # determine and set the learning rate for this iteration
        lr = get_lr(train.learning_rate, train_step, train.lr_warmup_steps, max_iters, train.min_lr)
        for param_group in optimizer.param_groups:
            param_group["lr"] = lr

        iter_t0 = time.perf_counter()

        input_ids = next(train_iter)["input_ids"]
        if isinstance(input_ids, (list, tuple)):
            input_ids = torch.stack(input_ids)
        input_ids = input_ids.to(fabric.device)

        # forward
        is_accumulating = (train_step + 1) % train.gradient_accumulation_iters != 0
        with fabric.no_backward_sync(model, enabled=is_accumulating):
            logits = model(input_ids)
            loss = chunked_cross_entropy(logits, input_ids, chunk_size=0)
            fabric.backward(loss / train.gradient_accumulation_iters)
        ...
        if not is_accumulating:
            ...
            if scheduler is not None:
                scheduler.step()
            train_step += 1
            state["iter_num"] = train_step

        running_loss.update(loss.detach())
        throughput.update(
            time=(time.perf_counter() - iter_t0),
            samples=train.micro_batch_size * devices * num_nodes,
            lengths=input_ids.numel(),
            flops_per_batch=measure_flops(model, input_ids),
        )

        if train_step % train.log_interval == 0:
            loss = running_loss.compute()
            t0 = time.time()
            fabric.print(
                f"iter {train_step}: loss {loss.item():.4f}, lr {lr:.2e}, throughput: {throughput.throughput.item():.2f} samples/s, "
                f"tokens/s: {throughput.tokens_per_sec:.2f}, flops/s: {throughput.flops_per_sec:.2f}, time elapsed: {t0 - state['t0']:.2f}s"
            )
            fabric.log_dict(
                {
                    "iter": train_step,
                    ...
                    "learning_rate": lr,
                    "throughput": throughput.throughput.item(),
                    "tokens_per_sec": throughput.tokens_per_sec.item(),
                    "flops_per_sec": throughput.flops_per_sec.item(),
                }
            )
            running_loss.reset()

        if train_step % train.save_interval == 0:
            checkpoint_file = out_dir / f"iter-{train_step:06d}.pt"
            save_checkpoint(fabric, state, tokenizer_dir, checkpoint_file)

        if eval.interval > 0 and (train_step % eval.interval == 0 or train_step == max_iters - 1):
            val_loss = validate(fabric, model, val_dataloader, eval.max_iters)
            fabric.print(f"iter {train_step}: val loss {val_loss:.4f}")
            fabric.log_dict({"iter": train_step, "val_loss": val_loss})
            if val_loss < best_val_loss:
                best_val_loss = val_loss
                best_checkpoint_file = out_dir / "best.pt"
                save_checkpoint(fabric, state, tokenizer_dir, best_checkpoint_file)
