...
class Roberta(nn.Module):
    def __init__(self, config: Config) -> None:
        ...
        # Roberta uses a different head for MLM vs LM
        self.lm_head = nn.Linear(config.n_embd, config.padded_vocab_size, bias=config.lm_head_bias)
        self.mlm_head = nn.Linear(config.n_embd, config.padded_vocab_size, bias=config.lm_head_bias)
    ...
    def forward(
        ...
        if input_pos is not None:
            ...
            if input_pos.dim() == 1:
                ...
                sin = sin.unsqueeze(0)
            if self.mask_cache is None:
                raise TypeError("You need to call `roberta.set_kv_cache()`")
            mask = batched_index_select(self.mask_cache, 2, input_pos)
            if mask.dim() > 4:
                mask = mask.view(*(mask.shape[0:1] + mask.shape[2:]))
            if input_pos_maxp1 is not None:
        ...
        if masked_lm_labels is not None:
            # For MLM pretraining
            if lm_head_chunk_size > 0:
                return [clamp_head(self.mlm_head(x_i)) for x_i in x.split(lm_head_chunk_size, dim=1)]
            return clamp_head(self.mlm_head(x))
        else:
            # For regular language modeling
            if lm_head_chunk_size > 0:
                return [clamp_head(self.lm_head(x_i)) for x_i in x.split(lm_head_chunk_size, dim=1)]
    ...
    def rope_cache(self, device: Optional[torch.device] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        return build_rope_cache(
            ...
            base=self.config.rope_base,
            condense_ratio=self.config.rope_condense_ratio,
            extra_config=self.config.rope_adjustments,
            rope_local_base_freq=self.config.rope_local_base_freq,
        )
    ...
    def set_kv_cache(
        ...
        if max_seq_length is None:
            max_seq_length = self.max_seq_length
        if rope_cache_length is None:
            rope_cache_length = max_seq_length
        self.max_seq_length = max_seq_length
        self.mask_cache = build_mask_cache(max_seq_length, device=device)
        for block in self.transformer.h:
            block.attn.set_kv_cache(batch_size, max_seq_length, rope_cache_length, device, dtype)
